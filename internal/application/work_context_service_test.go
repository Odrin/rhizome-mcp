...
func TestNewWorkContextServiceRejectsMissingRepository(t *testing.T) {
	if _, err := application.NewWorkContextService(nil); !errors.Is(err, &domain.Error{Code: domain.CodeInvalidArgument}) {
...
func TestWorkContextServiceNormalizesDelegatesAndClonesResult(t *testing.T) {
	...
	service, err := application.NewWorkContextService(repository)
	if err != nil {
		t.Fatal(err)
	}
