...
func TestMigrateEmptyDatabaseCreatesCompleteSchema(t *testing.T) {
	...
	if result != (Result{Version: CurrentVersion(), Applied: 7}) {
		t.Fatalf("Migrate() result = %+v, want current version with seven applied migrations", result)
	}
	...
	ordinaryTables := []string{
		...
		"review_events", "review_follow_ups", "review_outcomes", "review_requests", "review_targets", "schema_migrations", "work_attempts",
	}
	...
	if version != CurrentVersion() || name != "fixed_width_timestamps" || checksum != fixedWidthTimestampsChecksum {
		t.Fatalf("history = (%d, %q, %q), want current embedded migration", version, name, checksum)
	}
	actualChecksum := sha256.Sum256([]byte(fixedWidthTimestampsSQL))
...
func TestMigrateIsIdempotent(t *testing.T) {
	...
	if first.Applied != 7 || second != (Result{Version: CurrentVersion(), Applied: 0}) {
...
func TestMigrateUpgradesExistingRowsIntoSearchIndex(t *testing.T) {
	...
	if result != (Result{Version: CurrentVersion(), Applied: 6}) {
		t.Fatalf("upgrade result = %+v, want six applied migrations", result)
	}
...
func TestMigrateReviewContextUpgradePreservesHistory(t *testing.T) {
	...
	if result != (Result{Version: CurrentVersion(), Applied: 4}) {
		t.Fatalf("upgrade result = %+v, want four applied migrations", result)
	}
	...
	if len(after) != len(before)+4 {
		t.Fatalf("history rows = %d, want %d", len(after), len(before)+4)
	}
	...
	if after[len(after)-1].version != CurrentVersion() || after[len(after)-1].name != "fixed_width_timestamps" || after[len(after)-1].checksum != fixedWidthTimestampsChecksum {
		t.Fatalf("new history row = %+v, want fixed_width_timestamps migration", after[len(after)-1])
	}
	var count int
	if err := db.Read(ctx, func(ctx context.Context, query sqlite.Queryer) error {
...
func TestMigrateRejectsTamperedAndMalformedHistory(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "future", tamper: "INSERT INTO schema_migrations VALUES (8, 'future_schema', 'bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb', '2026-01-01T00:00:00Z')"},
