...
var expectedToolHints = map[string]sdkmcp.ToolAnnotations{
	...
	"unarchive_issue":       {ReadOnlyHint: false, DestructiveHint: boolPointer(true), IdempotentHint: true, OpenWorldHint: boolPointer(false)},
	...
	// ISSUE-174 workflow gates. manage_workflow_policy is destructive (archive
	// retires a policy irreversibly) and not idempotent (create without an
	// idempotency_key makes a new policy each call). submit_gate_evidence is an
	// upsert keyed by (attempt, key), so repeating it is idempotent and
	// destroys nothing.
	"manage_workflow_policy":     {ReadOnlyHint: false, DestructiveHint: boolPointer(true), IdempotentHint: false, OpenWorldHint: boolPointer(false)},
	"get_workflow_policy":        {ReadOnlyHint: true, DestructiveHint: boolPointer(false), IdempotentHint: true, OpenWorldHint: boolPointer(false)},
	"list_workflow_policies":     {ReadOnlyHint: true, DestructiveHint: boolPointer(false), IdempotentHint: true, OpenWorldHint: boolPointer(false)},
	"submit_gate_evidence":       {ReadOnlyHint: false, DestructiveHint: boolPointer(false), IdempotentHint: true, OpenWorldHint: boolPointer(false)},
	"evaluate_gates":             {ReadOnlyHint: true, DestructiveHint: boolPointer(false), IdempotentHint: true, OpenWorldHint: boolPointer(false)},
	...
	"create_review_request":      {ReadOnlyHint: false, DestructiveHint: boolPointer(false), IdempotentHint: false, OpenWorldHint: boolPointer(false)},
